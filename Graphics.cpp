 ...
 * Project 4: CoolPics
 *
 * Rishi Baroni, Allen Shen
 * rbaronia, shenal
 *
 * Cpp file for the graphics class, implements different functions of graphics.h
 */
...
void Graphics::setPixel(int x, int y, Color color) {
    ...
    // makes sure x and y are within bounds and sets specified pixel to color
    if (x >= 0 && x < DIMENSION) {
        if (y >= 0 && y < DIMENSION) {
            pixelData[y][x] = color;
        }
...
void Graphics::initArray() {
    ...
    // traverses through array and sets each pixel to black
    for (int i = 0; i < DIMENSION; i++) {
        for (int j = 0; j < DIMENSION; j++) {
            pixelData[i][j] = Color(0, 0, 0);
        }
